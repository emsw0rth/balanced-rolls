...
local COLOR_TITLE = { 0.59, 0.5, 0.82, 1 }

local function getHistory()
    return BR.LootHistory and BR.LootHistory.history or {}
end
...
function HistoryUI:Build()
    local Window = CreateFrame("Frame", "BalancedRollsLootHistoryWindow", UIParent, "BackdropTemplate")
    ...
    local ScrollFrame = CreateFrame("ScrollFrame", "BalancedRollsLootHistoryScrollFrame", Window, "UIPanelScrollFrameTemplate")
    ...
    ClearButton:SetScript("OnClick", function()
        StaticPopup_Show("BALANCED_ROLLS_LOOT_HISTORY_CLEAR_CONFIRM")
    end)
    ...
    StaticPopupDialogs["BALANCED_ROLLS_LOOT_HISTORY_CLEAR_CONFIRM"] = {
        text = "Are you sure you want to clear all loot history entries? This cannot be undone.",
        button1 = "Yes",
        button2 = "No",
        OnAccept = function()
            wipe(getHistory())
    ...
    tinsert(UISpecialFrames, "BalancedRollsLootHistoryWindow")
end
...
function HistoryUI:ShowExportWindow()
    ...
    local Window = CreateFrame("Frame", "BalancedRollsLootHistoryExportWindow", UIParent, "BackdropTemplate")
    ...
    Hint:SetPoint("TOPLEFT", Window, "TOPLEFT", 12, -32)
    Hint:SetTextColor(0.5, 0.5, 0.5, 1)
    Hint:SetText("Ctrl+A to select all, Ctrl+C to copy")

    local ScrollFrame = CreateFrame("ScrollFrame", "BalancedRollsLootHistoryExportScrollFrame", Window, "UIPanelScrollFrameTemplate")
    ScrollFrame:SetPoint("TOPLEFT", Window, "TOPLEFT", 12, -56)
    ScrollFrame:SetPoint("BOTTOMRIGHT", Window, "BOTTOMRIGHT", -32, 12)

    local EditBox = CreateFrame("EditBox", "BalancedRollsLootHistoryExportEditBox", ScrollFrame)
    ...
    tinsert(UISpecialFrames, "BalancedRollsLootHistoryExportWindow")
...
function HistoryUI:HistoryToCSV()
    ...
    for _, entry in ipairs(getHistory()) do
...
function HistoryUI:UpdateExportText()
    ...
    self.ExportEditBox:SetText(self:HistoryToCSV())
